...
class WantedCompanies(Resource):
    ...
    def put(self):
        """테그 추가 """
        return company_service.update_company(request.args.get('company_id'), request.args.get('col_target'),
                                              request.args.get('company_tag'))

    @api.expect(parser)
    def delete(self):
        """테그 삭제"""
        return company_service.delete_company(request.args.get('company_id'), request.args.get('col_target'),
                                              request.args.get('company_tag'))
